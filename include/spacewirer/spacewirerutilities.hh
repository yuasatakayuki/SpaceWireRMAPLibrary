...
class SpaceWireRUtilities {
 private:
  static const u16 CRC_INIT_VAL = 0xFFFFU;

 public:
  static u16 calculateCRCForArray(u8* data, size_t length) {
    static const u16 CRC16Table[] = {
        0x00,   0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7, 0x8108, 0x9129, 0xa14a, 0xb16b, 0xc18c, 0xd1ad,
    ...
    u16 result = CRC_INIT_VAL;
    for (size_t i = 0; i < length; i++) {
      result = (result << 8) ^ CRC16Table[static_cast<u8>(result >> 8) ^ *data++];
    }
 ...
 public:
  static u16 calculateCRCForHeaderAndData(std::vector<u8>& header, std::vector<u8>& data) {
    static const u16 CRC16Table[] = {
        0x00,   0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7, 0x8108, 0x9129, 0xa14a, 0xb16b, 0xc18c, 0xd1ad,
        0xe1ce, 0xf1ef, 0x1231, 0x210,  0x3273, 0x2252, 0x52b5, 0x4294, 0x72f7, 0x62d6, 0x9339, 0x8318, 0xb37b, 0xa35a,
        0xd3bd, 0xc39c, 0xf3ff, 0xe3de, 0x2462, 0x3443, 0x420,  0x1401, 0x64e6, 0x74c7, 0x44a4, 0x5485, 0xa56a, 0xb54b,
    ...
    // header
    length = header.size();
    std::vector<u8>::iterator it = header.begin();
    for (size_t i = 0; i < length; i++) {
      result = (result << 8) ^ CRC16Table[static_cast<u8>(result >> 8) ^ *it++];
    }

    // header
    if (data.size() != 0) {
      length = data.size();
      std::vector<u8>::iterator bytePointer2 = data.begin();
      for (size_t i = 0; i < length; i++) {
        result = (result << 8) ^ CRC16Table[static_cast<u8>(result >> 8) ^ *bytePointer2++];
      }
    }
