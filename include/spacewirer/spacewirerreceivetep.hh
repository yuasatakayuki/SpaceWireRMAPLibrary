...
class SpaceWireRReceiveTEP : public SpaceWireRTEP {
 public:
  ...
  std::vector<u8>* receive(double timeoutDuration = WaitDurationInMsForReceive) throw(SpaceWireRTEPException) {
  ...
  size_t getNDiscardedApplicationData() const { return nDiscardedApplicationData; }
  size_t getNDiscardedApplicationDataBytes() const { return nDiscardedApplicationDataBytes; }
  size_t getNDiscardedCommandPackets() const { return nDiscardedControlPackets; }
  size_t getNDiscardedHeartBeatPackets() const { return nDiscardedHeartBeatPackets; }
  size_t getNDiscardedDataPackets() const { return nDiscardedDataPackets; }
  ...
  inline u8 getSequenceNumberOfLastAck() { return sequenceNumberOfLastAck; }
  inline u8 getMaximumAcceptableSequenceNumber() {
    const u8 n = this->receiveSlidingWindowFrom;
    const u8 k = this->receiveSlidingWindowSize;
    const auto nkp1 = static_cast<u8>(n + k - 1);
    return (n < nkp1) ?
                      // n -- n + k - 1
               n + k - 1
                      :
  ...
  static constexpr size_t MaxReceivedApplicationData = 1000;
 ...
 private:
  ...
  void registerMeToSpaceWireREngine() { spwREngine->registerReceiveTEP(this); }
  void unregisterMeToSpaceWireREngine() { spwREngine->unregisterReceiveTEP(channel); }
  std::vector<u8>* popApplicationData() {
    std::vector<u8>* result = *(receivedApplicationData.begin());
    receivedApplicationData.pop_front();
    return result;
  }
  ...
  std::vector<u8>* currentApplicationData;
  bool isReceivingSegmentedApplicationData;
  std::list<std::vector<u8>*> receivedApplicationData;
  CxxUtilities::Condition receiveWaitCondition;  // used for receive of application data

  bool hasReceivedDataPacket;
  u8 sequenceNumberOfLastAck;
  u8 receiveSlidingWindowFrom;
  u8 receiveSlidingWindowSize;
  ...
  void controlPacketWasDiscarded() { nDiscardedControlPackets++; }
 ...
 private:
  ...
  bool errorInjectionNoReply(SpaceWireRPacket* packet) {
    using namespace std;
    u8 sequenceNumber = packet->getSequenceNumber();
  ...
  bool insideForwardReceiveSlidingWindow(u8 sequenceNumber) {
    const u8 n = this->receiveSlidingWindowFrom;
    const u8 k = this->receiveSlidingWindowSize;
    const auto nPluskMinus1 = static_cast<u8>(n + k - 1);
    if (n < nPluskMinus1) {
      // n -- n + k - 1
      return (n <= sequenceNumber && sequenceNumber <= nPluskMinus1);
    } else {
      // n -- 255, 0 -- n + k - 1
      return ((n <= sequenceNumber && sequenceNumber <= 255) ||
              (0 <= sequenceNumber && sequenceNumber <= nPluskMinus1));
    }
  }

 private:
  bool insideBackwardReceiveSlidingWindow(u8 sequenceNumber) {
    const u8 n = this->receiveSlidingWindowFrom;
    const u8 k = this->receiveSlidingWindowSize;
    const auto nMinusk = static_cast<u8>(n - k);
    const auto nMinus1 = static_cast<u8>(n - 1);
    if (nMinusk < nMinus1) {
      // n-k -- n-1
      return (nMinusk <= sequenceNumber && sequenceNumber <= nMinus1);
    } else {
      // n-k -- 255, 0 -- n - 1
      return (nMinusk <= sequenceNumber && sequenceNumber <= 255) ||
          (0 <= sequenceNumber && sequenceNumber <= nMinus1));
    }
  }
...
#ifdef DebugSpaceWireRReceiveTEP
      cout << "SpaceWireRReceiveTEP::consumeReceivedPackets() Received packet cannot be handled by the "
              "SpaceWireRReceiveTEP."
           << endl;
#endif
    }
#ifdef DebugSpaceWireRReceiveTEP
  ...
  void processDataPacket(SpaceWireRPacket* packet) {
    ...
    if (insideForwardReceiveSlidingWindow(sequenceNumber)) {
      ...
      std::stringstream ss;
      ss << "sequenceNumber:"
         << "0x" << hex << right << setw(2) << setfill('0') << (uint32_t)packet->getSequenceNumber() << endl;
      ...
      ss << "receiveSlidingWindowFrom:"
         << "0x" << hex << right << setw(2) << setfill('0') << (uint32_t)this->receiveSlidingWindowFrom << endl;
...
#endif
    u8 n = this->receiveSlidingWindowFrom;
  ...
  void reconstructApplicationData(SpaceWireRPacket* packet) {
    ...
    if (!packet->isDataPacket()) {
#ifdef DebugSpaceWireRReceiveTEP
      cout << "SpaceWireRReceiveTEP::reconstructApplicationData() received packet is not Data packet. Reconstruction "
              "is skipped."
           << endl;
#endif
      return;
    ...
    if (isReceivingSegmentedApplicationData) {
      ...
      if (packet->isFirstSegment()) {
        ...
        currentApplicationData = new std::vector<u8>;
        appendDataToCurrentApplicationDataInstance(packet);
        isReceivingSegmentedApplicationData = true;
        return;
      }
  ...
  void appendDataToCurrentApplicationDataInstance(SpaceWireRPacket* packet) {
    std::vector<u8>* payload = packet->getPayload();
  ...
  void completeServiceDataUnitHasBeenReceived(SpaceWireRPacket* packet) {
    std::vector<u8>* applicationData = new std::vector<u8>(packet->getPayloadLength());
  ...
  inline void setSequenceNumberOfLastAck(u8 sequenceNumber) { sequenceNumberOfLastAck = sequenceNumber; }
};
